...
export const getElapsedTimeInUAE = (date) => {
  ...
  const nowInUAE = formatInTimeZone(
    new Date(),
    UAE_TIMEZONE,
    "yyyy-MM-dd HH:mm:ss"
  );
...
export const getElapsedTimeInIndianTimeZone = (date) => {
  if (!date) return { hours: 0, minutes: 0, seconds: 0 };
...
export const formatToIndianTime = (date) =>
  formatInTimeZone(new Date(date), "Asia/Kolkata", "MMM dd, yyyy HH:mm");
